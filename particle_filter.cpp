 ...
 */
// Part of code referred from https://github.com/jeremy-shannon/CarND-Kidnapped-Vehicle-Project/blob/master/src/particle_filter.cpp
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  for(int i = 0; i < num_particles; i++) {
      if(fabs(yaw_rate) < 0.00001) {  // If yaw_rate is small, Small-angle approximation
          particles[i].x += velocity * delta_t * cos(particles[i].theta);
          particles[i].y += velocity * delta_t * sin(particles[i].theta);
      } 
      else{ // 5-9. Calculate Prediction Step Quiz Explanation
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for (unsigned int i = 0; i < observations.size(); i++) {
    ...
    for (unsigned int j = 0; j < predicted.size(); j++) {
      ...
      // get distance between current/predicted landmarks (function from helper_functions.h)
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    for (unsigned int j = 0; j < observations.size(); j++) {
      // Homogenous Transformation in 5-17. Landmarks Quiz Solution
    ...
    for (unsigned int j = 0; j < transformed_os.size(); j++) {
      ...
      // calculate weight for this observation with multivariate Gaussian (From 5-20. Particle Weights Solution)
...
void ParticleFilter::resample() {
  ...
  double beta = 0.0;
  // spin the resample wheel! (From 4-20. Resampling Wheel)
  for (int i = 0; i < num_particles; i++) {
